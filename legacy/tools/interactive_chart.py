...
def parse_trades(raw_data):
    ...
    for item in raw_data:
        ...
        # Always display Polymarket timestamps in US/Eastern, independent of local machine timezone.
        entry["dt"] = (
...
def main():
    ...
    et_tz = ZoneInfo("America/New_York")
    start_et = datetime.datetime.fromtimestamp(min_ts, tz=timezone.utc).astimezone(et_tz).replace(tzinfo=None)
    end_et = datetime.datetime.fromtimestamp(max_ts, tz=timezone.utc).astimezone(et_tz).replace(tzinfo=None)
