#!/usr/bin/env python3
"""Scan BTC adapter residual lots for strict L2 rescue-close opportunities."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_DATA_ROOT / "derived/contract_examples/btc_strict_rescue_opportunity_latest",
    )
    ...
    output_db = output_dir / "btc_strict_rescue_opportunity.duckdb"
    actions_csv = btc_adapter_dir / "actions.csv"
    residual_csv = btc_adapter_dir / "residual_lots.csv"
    ...
    try:
        ...
        con.execute(
            ...
            SELECT
              ...
              (t.bid1_px - ({float(args.fee_rate)} * t.bid1_px * (1.0 - t.bid1_px))) * r.qty AS after_fee_recovery_value,
        ...
        summary_dict = dict(zip(names, summary))
        con.execute(f"COPY strict_rescue_by_lot TO {quote(output_dir / 'btc_strict_rescue_by_lot.csv')} (HEADER, DELIMITER ',')")
        con.execute(
            f"COPY strict_rescue_by_lot TO {quote(output_dir / 'btc_strict_rescue_by_lot.parquet')} (FORMAT PARQUET, COMPRESSION ZSTD)"
        )
        con.execute("CHECKPOINT")
    finally:
        con.close()

    residual_count = int(summary_dict.get("residual_lot_count") or 0)
    break_even_count = int(summary_dict.get("break_even_after_fee_lots") or 0)
    beats_settlement_count = int(summary_dict.get("rescue_beats_settlement_lots") or 0)
    manifest = {
        "schema_version": "btc_strict_rescue_opportunity_report_v1",
        "created_utc": utc_now(),
        "status": "OK_BTC_STRICT_RESCUE_OPPORTUNITY_READY",
        "btc_adapter_dir": str(btc_adapter_dir),
        "l2_top_manifest": str(l2_manifest_path),
        "l2_top_status": l2_manifest.get("status"),
        "l2_top_table": l2_table,
        "lookahead_s": args.lookahead_s,
        "fee_rate": args.fee_rate,
        "outputs": {
            "duckdb": str(output_db),
            "table": "strict_rescue_by_lot",
            "csv": str(output_dir / "btc_strict_rescue_by_lot.csv"),
            "parquet": str(output_dir / "btc_strict_rescue_by_lot.parquet"),
        },
        "summary": {
            **summary_dict,
            "break_even_after_fee_lot_rate": (break_even_count / residual_count) if residual_count else None,
            "rescue_beats_settlement_lot_rate": (beats_settlement_count / residual_count) if residual_count else None,
        },
        "semantics": {
            "strict_rescue": "same condition_id and side, after seed timestamp, within lookahead window, top-aligned bid1 size covers residual qty, after-fee bid recovery computed with official fee formula",
            "research_only": True,
            "private_truth": False,
        },
    }
    manifest_path = output_dir / "BTC_STRICT_RESCUE_OPPORTUNITY_REPORT.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"status": manifest["status"], "summary": manifest["summary"], "outputs": manifest["outputs"]}, indent=2, sort_keys=True))
    return 0
