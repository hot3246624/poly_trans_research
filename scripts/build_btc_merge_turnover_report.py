...
def main() -> int:
    ...
    report = {
        ...
        "metrics": {
            ...
            "paired_mergeable_qty": rounded(pair_qty),
            "paired_mergeable_cost": rounded(pair_cost_sum),
            "pair_merge_redeem_value": rounded(pair_qty),
            "merge_recovered_capital": rounded(pair_qty),
            ...
            "capital_turnover": rounded(pair_qty / gross_buy_cost) if gross_buy_cost else None,
            "residual_cost": rounded(residual_cost),
            "residual_qty": rounded(float(metrics.get("residual_qty") or 0.0)),
            "market_end_residual_qty": rounded(float(metrics.get("residual_qty") or 0.0)),
            ...
            "actual_settlement_residual_pnl": rounded(float(metrics.get("residual_settle_pnl") or 0.0)),
            "residual_zero_stress_loss": rounded(-residual_cost),
            "strict_rescue_recovery_value": rounded(rescue_recovery),
            "strict_rescue_pnl": rounded(rescue_residual_pnl),
        ...
        "semantics": {
            ...
            "paired_mergeable_qty": "min(YES inventory, NO inventory) matched inside the BTC adapter pair cycle.",
            "merge_recovered_capital": "Matched pair quantity redeemable at 1.0; this is capital reuse/turnover attribution.",
