...
OUTCOME_REPORT_PATH="${POLYTRANS_OUTCOME_REPORT_PATH:-$ROOT_DIR/data/replay/audits/outcome_backfill_${TARGET_DAY}.json}"
VALIDATE_GAP_THRESHOLD_MS="${POLYTRANS_VALIDATE_GAP_THRESHOLD_MS:-0}"

RAW_DAY_DIR="$ROOT_DIR/data/raw/$TARGET_DAY"
PUBLISHED_DB="$ROOT_DIR/data/replay_published/$TARGET_DAY/crypto_5m.sqlite"
...
run_cycle() {
  "$UV_BIN" run python cfdata.py --log-level INFO build-replay --day "$TARGET_DAY" --replay-root data/replay
  "$UV_BIN" run python cfdata.py --log-level INFO validate-replay --day "$TARGET_DAY" --replay-root data/replay --gap-threshold-ms "$VALIDATE_GAP_THRESHOLD_MS"
